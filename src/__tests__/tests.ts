...
describe("POST /api/v1/user/todos", () => {
  ...
  it("Should return a 400 status code if the title is missing", async () => {
    const resp = await request(app)
      .post("/api/v1/user/todos")
      .send({
  ...
  it("Should return a 400 status code if the description is missing", async () => {
    const resp = await request(app)
      .post("/api/v1/user/todos")
      .send({
        title: "Test Title",
  ...
  it("Should return a 500 status code and a generic error message when an unhandled exception occurs", async () => {
    jest.spyOn(TodoService, "createTodo").mockImplementation(() => {
      throw new Error("Unhandled Exception");
    });
